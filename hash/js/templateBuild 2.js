$(document).ready(function () {
	...
	$(Slider).on('mouseup touchend', function(){
	  var words = document.getElementById('wordsInput').value;
		...
		t = $(t)[0].getElementsByClassName("textBox")[0];
    ...
    for (var i = 0; i < targetDivs.length; i++) {
      ...
      if (cb == true) {
		...
		var DDMMYY = DD+MM+YY;
          ...
          a.download = '9NEWSMELBOURNE_' + DDMMYY + '_' + canvas.width + 'x' + canvas.height + '.jpeg';
          a.click();
...
function updateAllWords(words){
	...
	for (var i = 0; i < targetDivs.length; i++) {
      ...
      var e = targetDivs[i].getElementsByClassName("textBox")[0];
...
function sliders(e){
	...
	//setTimeout(function () {$(t).css("background",'rgba(255,255,255,0.0)');}, 	1000);
